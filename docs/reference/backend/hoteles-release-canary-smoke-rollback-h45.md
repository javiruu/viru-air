...
| Worker image | `backend/Dockerfile`, `backend/uv.lock` y `infra/github/workflows/ci.yml` | build multi-stage basado en lock **construido y validado en contenedor** (exit 0; `import app.main` OK; uvicorn `/health` → 200 con DB aislada migrada); CI lo construye sin push | no demuestra una imagen publicada, un tag/digest inmutable aprobado ni un despliegue real |
