...
| Worker image | `backend/uv.lock`, `infra/github/workflows/ci.yml` y `infra/github/workflows/release.yml` | build basado en lock **construido y validado localmente** (exit 0; `import app.main` OK; uvicorn `/health` → 200 con DB aislada migrada); CI construye sin push; `release.yml` incluye el job `publish-image` preparado (tags `sha-<commit>`/`latest`, `packages: write`) | no demuestra que el publish se haya ejecutado contra GHCR: imagen publicada, digest aprobado y despliegue real quedan por demostrar |
