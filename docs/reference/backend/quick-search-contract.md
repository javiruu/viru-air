...
Warnings can�nicos esperados en `meta.warnings_structured`:
...
**Diferencias clave entre los tres conceptos de reutilizaci�n:**
...
| **Respuesta final recompuesta** | `query_signature` (qsig_*) | Payload completo del endpoint tras ranking, dedupe y paginaci�n | No se cachea como payload bruto can�nico | N/A (se recompone desde unidades) |
| **Snapshots de watchlist** | `(watch_id, captured_at_utc)` | Precio can�nico para un watch concreto | Indefinida (hist�rico) | Single-user (asociado a FlightWatch) |

### TTL por categor�a de resultado

- `ready`: **24h** (86400s) � resultados con vuelos v�lidos
- `empty`: **2h** (7200s) � b�squeda sin vuelos encontrados
...
### Reutilizaci�n para b�squedas ampliadas

Cuando una b�squeda usa `include_nearby` o `flex`, el backend:
1. Descompone la b�squeda en unidades exactas `(origin, destination, date, provider)`
2. Consulta la cache compartida para cada unidad
3. Solo llama al provider para las unidades no cacheadas o expiradas
4. Recompone la respuesta final (ranking, dedupe, paginaci�n) desde las unidades resueltas
...
- `FARE_MEMORY_WATCHLIST_BACKFILL_ENABLED=false` � activa backfill hist�rico al crear/guardar watches
- `FARE_MEMORY_BOOT_WARMUP_ENABLED=false`
- `FARE_MEMORY_MAX_BOOT_JOBS=25`
- `FARE_MEMORY_PROVIDER_RATE_LIMIT_PER_MINUTE=60`
- `REDIS_URL` � opcional; si est� configurada y operativa, habilita Redis como hot layer compartida
- `QUICK_SEARCH_REDIS_TTL_SECONDS=300` � TTL m�ximo de la hot layer Redis
- `QUICK_SEARCH_REDIS_MAX_PAYLOAD_BYTES=65536` � l�mite por payload Redis; si se supera, se omite solo la hot layer y DB sigue siendo fuente durable
...
- `stale_served_count`: resultados devueltos con `stale_data=true`, `requires_revalidation=true` o `freshness.status` c�lido/caducado
...
- Primera b�squeda live sin cache: `provider_calls=1`, `cache_misses=1`, `provider_calls_avoided=0`.
- Repetici�n con hit L1/L2: `provider_calls=0`, `cache_hits=1`, `provider_calls_avoided=1`.
...
La arquitectura est� dise�ada para que Redis pueda a�adirse como capa L1 (caliente) sin cambiar la fuente de verdad (BD). La cache en BD seguir�a siendo el contrato can�nico; Redis ser�a una aceleraci�n con TTL m�s corto.

## Implementation status (Junio 2026)

La cache compartida persistente (V2.1) est� implementada con las siguientes piezas:

| Componente | Archivo | Estado |
|---|---|---|
| Modelo DB | `backend/app/infrastructure/db/models.py` ? `QuickSearchCacheEntry` | ? vivo |
| Esquema | `backend/app/infrastructure/db/models.py` (tabla `quick_search_cache_entry`, aplicada con `create_all` — sin Alembic, ADR 0003) | ? vivo |
| Servicio de cache | `backend/app/services/quick_search_cache_service.py` | ? vivo |
| Canonicalizaci�n | `backend/app/services/quick_search_execution.py` ? `build_unit_cache_key`, `build_cache_source_hash`, `classify_cache_result` | ? vivo |
| Integraci�n execution | `backend/app/services/quick_search_execution.py` ? `execute_plan` + `_fetch_with_cache` (L1?L2?provider) | ? vivo |
| Anti-stampede | `backend/app/services/quick_search_execution.py` ? per-key `threading.Lock` en `_fetch_with_cache` | ? vivo |
| Integraci�n watchlist | `backend/app/api/v1/watchlist.py` ? `_refresh_watch_now` consulta y persiste cache | ? vivo |
...
La arquitectura est� dise�ada para a�adir Redis como capa L1 (caliente) sin cambiar la fuente de verdad (BD). La cache en BD sigue siendo el contrato can�nico; Redis ser�a una aceleraci�n con TTL m�s corto.
...
Cada b�squeda v�lida mantiene dos se�ales an�nimas:

- `quick_search_popularity_counter`: acumulado hist�rico por ruta, fecha de viaje y moneda;
- `quick_search_popularity_daily`: bucket por fecha de b�squeda, ruta direccional y moneda.

El segundo permite calcular una ventana real de siete d�as sin reinterpretar
`last_searched_at` ni atribuir el acumulado hist�rico a la semana actual. La
...
`/api/v1/community/routes/*`, por lo que una degradaci�n comunitaria no bloquea
la b�squeda principal.
