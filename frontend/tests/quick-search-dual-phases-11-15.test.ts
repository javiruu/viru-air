...
const UTILS_DUAL = path.join(process.cwd(), "src", "modules", "quick-search", "utils-dual.ts");
...
test("Phase 11: outbound panel uses per-side emptyCausesExpanded", () => {
  ...
  assert.match(
    ...
    "outbound must use outboundEmptyCausesExpanded",
  );
});
...
test("Phase 11: return panel uses per-side emptyCausesExpanded", () => {
  ...
  assert.match(
    ...
    "return must use returnEmptyCausesExpanded",
  );
});
...
test("Phase 11: outbound toggle wired to setOutboundEmptyCausesExpanded", () => {
  ...
  assert.match(
    source,
    /onToggleEmptyCauses=\{\(\) => setOutboundEmptyCausesExpanded/,
    "outbound toggle must wire correctly",
  );
});
...
test("Phase 11: return toggle wired to setReturnEmptyCausesExpanded", () => {
  ...
  assert.match(
    source,
    /onToggleEmptyCauses=\{\(\) => setReturnEmptyCausesExpanded/,
    "return toggle must wire correctly",
  );
});
...
test("Phase 11: outbound onRelaxAction wires to handleDualRelaxAction", () => {
  ...
  assert.match(
    source,
    /onRelaxAction=\{[\s\S]*?handleDualRelaxAction[\s\S]*?"outbound"[\s\S]*?\}/,
    "outbound relax must wire correctly",
  );
});
...
test("Phase 11: return onRelaxAction wires to handleDualRelaxAction", () => {
  ...
  assert.match(
    source,
    /onRelaxAction=\{[\s\S]*?handleDualRelaxAction[\s\S]*?"return"[\s\S]*?\}/,
    "return relax must wire correctly",
  );
});
...
test("Phase 11: increase_duration skips re-search (view-only filter)", () => {
  ...
  assert.match(
    ...
    "handler must check for increase_duration",
  );
  assert.match(
    handlerBody,
    /return;\s*\}/,
    "increase_duration block must have early return to skip re-search",
  );
});
...
test("Phase 12: combined price returns null when either price is missing", () => {
  ...
  assert.match(
    ...
    "must return null (not 0) for missing prices",
  );
});
...
test("Phase 13: fetchDeepLink accepts optional dateIn", () => {
  ...
  assert.match(
    sideSource,
    /query\.set\("date_in",\s*params\.dateIn\)/,
    "must pass dateIn to query",
  );
});
...
test("Phase 14: return side weather is explicitly null in dual mode", () => {
  ...
  assert.match(
    source,
    /weatherDestination=\{null\}.*Phase 14/,
    "return weatherDestination must be null",
  );
});
...
test("Phase 15: isDualMode excludes originCountryOnly and destinationCountryOnly", () => {
  ...
  assert.match(
    source,
    /!originCountryOnly\s*&&\s*!destinationCountryOnly/,
    "isDualMode must exclude country-only",
  );
});
...
test("Phase 15: dual submit inverts route scopes for return leg", () => {
  ...
  assert.match(
    ...
    "return leg must use destination scope as origin",
  );
  assert.match(
    source,
    /destination:\s*originRequestValue,/,
    "return leg must use origin scope as destination",
  );
});
...
test("Phase 15: utils-dual buildDualSearchParams maps all fields correctly", () => {
  ...
  assert.match(
    ...
    "must export buildDualSearchParams",
  );
...
test("Phase 15: utils-dual findCombinationResult handles empty results", () => {
  ...
  assert.match(
    ...
    "must export findCombinationResult",
  );
