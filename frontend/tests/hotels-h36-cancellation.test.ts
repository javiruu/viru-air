...
const SNAPSHOTS_COMPONENT = path.join(
  process.cwd(),
  "src",
  "modules",
  "hotels",
  "components",
  "HotelTrackedOfferSnapshots.tsx",
);
...
test("H36: caller abort remains distinguishable when timeout composition is enabled", async () => {
  ...
  try {
    const request = apiFetchWithStatus(
      "/hotels/search",
...
test("H36: tracked snapshot history accepts a caller signal", async () => {
  ...
  try {
    ...
    await assert.rejects(
      request,
      (error: unknown) => error instanceof HotelsRequestError && error.status === 0,
    );
    assert.equal(signalRef.current, controller.signal);
  } finally {
    globalThis.fetch = originalFetch;
...
test("H36: hotel hooks cancel obsolete search, resolve, and detail work", () => {
  ...
  assert.match(
    searchHook,
    /handleSearchModeChange[\s\S]*?areaResolveAbortRef\.current\?\.abort\(\)/,
  );
