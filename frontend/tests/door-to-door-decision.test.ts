...
test("decision badges are deterministic and mark the right winners", () => {
  const a = buildOption({ id: "a", label: "A", total_duration_minutes: 300, total_price_min: 80, transfer_count: 2, risk_level: "medium" });
  const b = buildOption({ id: "b", label: "B", total_duration_minutes: 270, total_price_min: 90, transfer_count: 1, risk_level: "low" });
  const c = buildOption({ id: "c", label: "C", total_duration_minutes: 310, total_price_min: 70, transfer_count: 3, risk_level: "high" });

  const badges = getDecisionBadges([a, b, c]);

  assert.ok((badges.b ?? []).some((item) => item.kind === "fastest"));
  assert.ok((badges.b ?? []).some((item) => item.kind === "lowest_risk"));
  assert.ok((badges.b ?? []).some((item) => item.kind === "fewest_changes"));
  assert.ok((badges.c ?? []).some((item) => item.kind === "best_estimated_price"));
});

test("decision reasons prioritize price, buffer, and risk when recommendation leads", () => {
  const recommended = buildOption({ id: "r", label: "R", total_price_min: 70, total_duration_minutes: 280, airport_buffer_minutes: 150, transfer_count: 1, risk_level: "low", is_recommended: true });
  const alt = buildOption({ id: "x", label: "X", total_price_min: 85, total_duration_minutes: 300, airport_buffer_minutes: 110, transfer_count: 2, risk_level: "medium" });

  const reasons = getDecisionReasons(recommended, [recommended, alt]);
  assert.deepEqual(reasons.map((item) => item.kind), ["price", "buffer", "risk"]);
});
...
test("decision reasons include confidence cue for deeplink/estimate data", () => {
  ...
  const alt = buildOption({ id: "x", label: "X", total_price_min: null, risk_level: "medium" });
...
test("alternative deltas compare against recommendation with null-safe fields", () => {
  const recommended = buildOption({ id: "r", label: "R", total_price_min: 80, total_duration_minutes: 300, airport_buffer_minutes: 120, risk_level: "medium", score: 90 });
  const alt = buildOption({ id: "x", label: "X", total_price_min: 95, total_duration_minutes: 280, airport_buffer_minutes: 100, risk_level: "high", score: 70 });
  ...
  assert.equal(deltas[0].risk_change, "worse");
});
