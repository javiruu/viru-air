...
import {
  ...
  getDecisionBadges,
  getDecisionReasons,
} from "../src/modules/door-to-door/decision";
import type { DoorToDoorOption } from "../src/modules/door-to-door/types";

function buildOption(
  partial: Partial<DoorToDoorOption> & Pick<DoorToDoorOption, "id" | "label">,
): DoorToDoorOption {
  return {
    id: partial.id,
    label: partial.label,
    ...
    completeness: partial.completeness ?? "full",
...
test("decision reasons prioritize price, buffer, and transfers when recommendation leads", () => {
  ...
  assert.deepEqual(
    reasons.map((item) => item.kind),
    ["price", "buffer", "transfers"],
  );
});
...
test("alternative deltas rank safer options ahead of tight-buffer options with inflated raw score", () => {
  ...
  const riskyAlt = buildOption({
    id: "risk",
    label: "Risky",
    total_price_min: 70,
    total_duration_minutes: 250,
    airport_buffer_minutes: 50,
    transfer_count: 1,
    score: 88,
  });
  const safeAlt = buildOption({
    id: "safe",
    label: "Safe",
    total_price_min: 92,
    total_duration_minutes: 285,
    airport_buffer_minutes: 125,
    transfer_count: 2,
    score: 84,
  });
