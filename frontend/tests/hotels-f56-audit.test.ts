...
const HOTELS_SIGNALS = path.join(
  process.cwd(),
  "src",
  "modules",
  "hotels",
...
test("H56: HotelRadarPage keeps the post-close composition and section order", () => {
  ...
  assert.ok(
    source.indexOf("<HotelTrackedOffersPanel") >
      source.indexOf("className={`panel panel-soft hotel-results-panel"),
  );
...
test("H32: hotel errors and collapsible panels expose accessible semantics", () => {
  ...
  for (const panel of ["detail", "parity", "alerts", "compSet"]) {
    assert.match(
      page,
      new RegExp(`aria-controls="hotel-${panel === "compSet" ? "compset" : panel}-panel"`),
    );
  }
});
...
test("H32: hotel mobile controls preserve 48px targets and long-content wrapping", () => {
  ...
  assert.match(
    css,
    /@media \(max-width:\s*768px\)[\s\S]*\.hotel-overview-strip \{[^}]*overflow-x:\s*auto/s,
  );
  assert.match(css, /\.hotel-provider-toggle-row \{[^}]*min-height:\s*48px/);
  assert.match(
    css,
    /\.hotel-result-main \{[^}]*display:\s*flex[^}]*width:\s*100%[^}]*min-height:\s*48px[^}]*overflow-wrap:\s*anywhere/,
  );
  assert.match(
    css,
    /\.hoteles-layout,\s*\.hoteles-main-column,\s*\.hoteles-side-column,\s*\.hotel-search-panel,\s*\.hotel-results-panel,\s*\.hotel-results-list \{\s*min-width:\s*0;/,
  );
  ...
  assert.match(
    css,
    /\.hoteles-page \.hotel-alerts-panel \.qs-input-neutral \{[\s\S]*width:\s*100%;[\s\S]*max-width:\s*100%;[\s\S]*min-width:\s*0;/,
  );
  assert.match(
    css,
    /\.hotel-area-spinner,[\s\S]*\.hotel-provider-toggle-row \{[\s\S]*animation:\s*none\s*!important/,
  );
});
...
test("H57: hotel result cards keep both tracking and watchlist actions visible", () => {
  ...
  assert.ok(
    source.indexOf('t("hotels.actions.addToWatchlist")') >
      source.indexOf('t("hotels.actions.trackPrice")'),
  );
});
