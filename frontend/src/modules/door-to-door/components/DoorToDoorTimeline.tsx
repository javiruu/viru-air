  ...
  return (
    <section className="panel panel-soft d2d-timeline-panel">
      <div className="panel-header">
        ...
        {flight?.flight_time_confidence === "estimated" ? (
          ...
          {legs.map((leg, index) => (
            <li key={`${leg.type}-${leg.mode}-${index}`}>
              ...
              <div>
                ...
                <p>
                  {leg.mode === "flight"
                    ? t("doorToDoor.timeline.flight")
                    : t("doorToDoor.timeline.ground")}
                  {" · "}
                  {leg.duration_minutes != null
                    ? `${leg.duration_minutes} min`
                    : t("doorToDoor.option.durationUnconfirmed")}
                  {" · "}
                  {timeRangeLabel(
                    leg.departure_at,
                    leg.arrival_at,
                    t("doorToDoor.option.scheduleUnconfirmed"),
                  )}
                </p>
              </div>
            </li>
