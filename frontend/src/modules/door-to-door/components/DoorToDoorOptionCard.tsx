...
export function DoorToDoorOptionCard({
  ...
  const hasGtfsSchedule = option.legs.some(
    (leg) =>
      leg.type === "ground" &&
      leg.source_type === "open_data" &&
      leg.provider === "gtfs_transit" &&
      leg.departure_at != null &&
      leg.arrival_at != null,
  );
  ...
  const legActions = option.legs
    ...
    .slice(0, 4);
  const transparencyNote = isRealDeeplink
    ...
    : isEstimate || isExploratory
      ...
      : isPartialActionable
        ...
        : null;
  const primarySource =
    option.sources.find((source) => source.booking_url) ?? option.sources[0] ?? null;
  ...
  function statusBadge() {
    if (isRealResult)
      return (
        <span className="status-pill success d2d-badge">{t("doorToDoor.option.realResult")}</span>
      );
    if (isRealDeeplink)
      return (
        <span className="status-pill info d2d-badge">{t("doorToDoor.option.realDeeplink")}</span>
      );
    return (
      <span className="status-pill warning d2d-badge">{t("doorToDoor.option.estimateOnly")}</span>
    );
  }
  ...
  function completenessBadge() {
    if (isFull)
      return (
        <span className="status-pill success d2d-badge">
          {t("doorToDoor.option.completenessFull")}
        </span>
      );
    if (isPartialActionable)
      return (
        <span className="status-pill info d2d-badge">
          {t("doorToDoor.option.completenessPartial")}
        </span>
      );
    return (
      <span className="status-pill warning d2d-badge">
        {t("doorToDoor.option.completenessExploratory")}
      </span>
    );
  }

  function priceLabel() {
    if (!hasConfirmedPrice(option.total_price_min, option.total_price_max))
      return t("doorToDoor.option.noPrice");
    const minPrice = option.total_price_min!;
    const maxPrice = option.total_price_max!;
    if (minPrice === maxPrice)
      return t("doorToDoor.option.fromPrice", { price: minPrice, currency: option.currency });
    return t("doorToDoor.option.estimatedRange", {
      min: minPrice,
      max: maxPrice,
  ...
  return (
    <article
      className={`d2d-option-card ${isRealResult ? "is-real" : ""} ${isRealDeeplink ? "is-deeplink" : ""} ${isEstimate ? "is-estimate" : ""} ${chosen ? "is-chosen" : ""} ${isRecommended ? "is-recommended" : ""} ${compact ? "is-compact" : ""}`}
    >
      {isRecommended ? (
        <span className="d2d-recommended-star" aria-label={t("doorToDoor.option.recommended")}>
          ★
        </span>
      ) : null}
      <div className="d2d-option-main">
        ...
        {reasons.length > 0 ? (
          <section
            className="d2d-decision-reasons"
            aria-label={t("doorToDoor.option.whyThisRouteTitle")}
          >
            <strong>{t("doorToDoor.option.whyThisRouteTitle")}</strong>
            <ul>
              {reasons.map((reason) => (
                <li key={`${option.id}-${reason.kind}`}>
                  {t(`doorToDoor.option.reasons.${reason.label}`)}
                </li>
              ))}
            </ul>
          </section>
        ...
        <div className="d2d-option-meta">
          {hasGoogleRoutes ? (
            <span className="status-pill info d2d-badge">
              {t("doorToDoor.option.realDuration")}
            </span>
          ) : null}
          {hasGtfsSchedule ? (
            <span className="status-pill success d2d-badge">
              {t("doorToDoor.option.openDataSchedule")}
            </span>
          ) : null}
          {hasDuration ? (
            <span>
              {t("doorToDoor.option.buffer", {
                minutes:
                  option.airport_buffer_minutes != null
                    ? option.airport_buffer_minutes
                    : t("doorToDoor.option.bufferUnconfirmed"),
              })}
              {!compact && isTightBuffer ? (
                <span className="status-pill warning d2d-badge" style={{ marginLeft: 6 }}>
                  {t("doorToDoor.option.bufferRiskLabel")}
                </span>
              ) : null}
            </span>
          ) : null}
          ...
          {hasPrice ? (
            ...
            <span className="d2d-option-price-unconfirmed">
              {isRealDeeplink
                ? t("doorToDoor.option.externalPriceNote")
          ...
          {option.sources.slice(0, 2).map((source) => {
            const sourceLabel =
              source.source_type === "maps"
                ? t("doorToDoor.source.maps")
                : source.source_type === "deeplink"
                  ? t("doorToDoor.source.deeplink")
                  : source.source_type === "open_data"
                    ? t("doorToDoor.source.openData")
                    : source.source_type === "estimate"
                      ? t("doorToDoor.source.estimate")
                      : source.source_type === "mock"
                        ? t("doorToDoor.source.mock")
                        : source.source_type;
            const tone =
              source.source_type === "maps"
                ? "info"
            ...
            return (
              <span
                key={`${option.id}-${source.provider}`}
                className={`status-pill ${tone} d2d-badge`}
              >
                {sourceLabel}
              </span>
            );
