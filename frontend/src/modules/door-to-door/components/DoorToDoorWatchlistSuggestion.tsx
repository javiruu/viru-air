...
export function DoorToDoorWatchlistSuggestion({ watch }: { watch: Watch }) {
  ...
  return (
    <section
      className="panel panel-soft d2d-watch-suggestion"
      aria-label={t("doorToDoor.watchSuggestion.aria")}
    >
      <div className="d2d-watch-suggestion-copy">
        <span className="d2d-mini-kicker">{t("doorToDoor.watchSuggestion.kicker")}</span>
        <strong>
          {t("doorToDoor.watchSuggestion.title", {
      ...
      <Link
        ...
        href={`/puerta-a-puerta?watchId=${encodeURIComponent(watch.id)}`}
      >
