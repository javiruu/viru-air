...
export function HotelWatchlistPanel({
  ...
  return (
    <section className="panel panel-soft hotel-watchlist-panel">
      ...
      {loading ? (
        <p className="panel-note section-gap-sm">{t("hotels.watchlist.loading")}</p>
      ) : null}
      {error ? <p className="panel-note section-gap-sm">{error}</p> : null}
      {!loading && !error && entries.length === 0 ? (
        <p className="panel-note section-gap-sm">{t("hotels.watchlist.empty")}</p>
      ) : null}
              ...
              <article key={item.id} className="list-row hotel-watchlist-item">
                <div className="hotel-watchlist-copy">
                  ...
                  <p className="panel-note">
                    {t("hotels.watchlist.savedAt", {
                      date: formatCreatedAt(item.created_at, localeTag),
                    })}
                  </p>
                  {detailUnavailable ? (
                    <p className="panel-note">{t("hotels.watchlist.detailUnavailable")}</p>
                  ) : null}
                </div>
                <div className="hotel-watchlist-actions">
                  <button
                    ...
                    onClick={() => onRemove(item.id, item.hotel_id)}
                    disabled={busy}
                  >
