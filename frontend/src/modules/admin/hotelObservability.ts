...
export const HOTEL_HEALTH_STATUSES = [
  "unknown",
  "not_configured",
  "ok",
  "degraded",
...
export type HotelRunDiagnosticStatus =
  | "running"
  | "completed"
  | "partial"
  | "failed"
...
export type HotelProviderControlOperation =
  | "ingestion"
  | "revalidation"
  | "area_search"
  | "unknown";
...
export type HotelSweepLeaseState =
  | "queued"
  | "running"
  | "expired"
  | "done"
...
export function getHotelLeaseTone(
  state: HotelSweepLeaseState,
): "success" | "warning" | "error" | "info" {
  if (state === "done") return "success";
  if (state === "expired" || state === "partial" || state === "skipped" || state === "running")
    return "warning";
  if (state === "failed") return "error";
  return "info";
}

export function getHotelCircuitTone(
  status: HotelProviderCircuit["status"],
): "success" | "warning" | "error" | "info" {
...
export function getHotelRunTone(
  status: HotelRunDiagnosticStatus,
): "success" | "warning" | "error" | "info" {
...
export function getHotelHealthTone(
...
export function getHotelOutcomesForMetric(
  metricName: HotelMetricName | "",
): readonly HotelOutcome[] {
...
export function buildHotelObservabilitySummary(
  metrics: readonly HotelDailyMetric[],
): HotelObservabilitySummary {
    ...
    attentionCount: metrics
      .filter(
        (metric) =>
          ...
          metric.outcome === "partial" ||
          metric.outcome === "retried",
      )
