...
function QuickSearchResultsListInner(props: Props) {
  ...
  const [internalFareProfile, setInternalFareProfile] = useState(() =>
    createEmptyFareComparisonProfile(travelers),
  );
  ...
  return (
    <>
      {props.visibleResults.length > 0 ? (
        <>
          <FareComparisonPanel profile={fareProfile} locale={locale} onChange={setFareProfile} />
          <div className={`qs-results-list ${props.compactView ? "compact" : ""}`}>
            {props.visibleResults.map((r, idx) => {
              const rowId = props.resultKey(r, idx);
              const provider = resolveQuickSearchProviderPresentation(
                r.source,
                props.t("sourceUnknown"),
              );
              ...
              const rowLink =
                provider.id === "ryanair"
                  ...
                  : provider.id === "wizzair"
                    ? isOfficialWizzAirDeepLink(r.deeplink_url)
                      ? r.deeplink_url
                      : ""
                    : isGenericHttpLink(r.deeplink_url)
                      ? (r.deeplink_url ?? "")
                      : "";
              const expanded = Boolean(props.expandedRows[rowId]);
              const detailsId = `details-${rowId}`;
              const aiReason =
                typeof r.ai_preferred_reason === "string" ? r.ai_preferred_reason.trim() : "";
              ...
              const departureClock =
                r.departure_time_local || formatLegClock(firstLeg?.dep_ts, props.localeTag);
              const arrivalClock =
                formatLegClock(lastLeg?.arr_ts, props.localeTag) ??
                addMinutesToClock(departureClock, totalDurationMinutes);
              const flightTimeLabel =
                totalDurationMinutes !== null ? props.formatMinutes(totalDurationMinutes) : null;
              ...
              const recommendationMark = r.ai_preferred ? (
                ...
                >
                  ...
                  <span
                    className="qs-result-recommendation-tooltip"
                    id={recommendationTooltipId}
                    role="tooltip"
                  >
                    {recommendationLabel}
                  </span>
                ...
                >
                    ...
                    {props.compactView ? (
                      <>
                        ...
                        <div className="qs-result-meta qs-result-meta-compact">
                          ...
                          <span>
                            {props.t("weatherDepart")} {departureClock || "--"}
                          </span>
                          {arrivalClock ? (
                            <span>
                              {props.t("weatherArrive")} {arrivalClock}
                            </span>
                          ) : null}
                        </div>
                      ...
                      <>
                        ...
                        <div className="qs-result-meta">
                          ...
                          {departureClock ? (
                            <span>
                              <strong>{props.t("weatherDepart")}:</strong> {departureClock}
                            </span>
                          ) : null}
                          {arrivalClock ? (
                            <span>
                              <strong>{props.t("weatherArrive")}:</strong> {arrivalClock}
                            </span>
                          ) : null}
                        </div>
                        <div className="qs-result-provider">
                  ...
                  <div className="qs-result-actions">
                    <div className="qs-result-price">
                      {!props.compactView ? (
                        <span className="qs-result-kicker">{props.t("resultsColPrice")}</span>
                      ) : null}
                      ...
                      {hasSelectedExtras ? (
                        <span
                          className={
                            fare.is_complete
                              ? "qs-result-comparable-note"
                              : "qs-result-comparable-note qs-result-comparable-note--missing"
                          }
                        >
                          {fare.is_complete
                      ...
                      {!props.compactView && r.ranking_score ? (
                        <span>
                          {props.t("score")} {props.formatScore(r.ranking_score)}
                        </span>
                      ) : null}
                    </div>
                    <div className="qs-result-buttons">
                      {props.isInWatchlist(r) ? (
                        <a
                          className="btn-ghost qs-row-saved"
                          href={watchlistHref}
                          onClick={() => props.viewInWatchlist(r)}
                        >
                        ...
                        <button
                          className="btn-secondary qs-row-save"
                          type="button"
                          onClick={() => props.addToWatchlist(r, fareProfile)}
                        >
                          {props.t("save")}
                        </button>
                      )}
                          ...
                          {props.openRowMenuId === rowId ? (
                            ...
                            >
                              ...
                              {canRefreshPrice ? (
                                ...
                                >
                                  {isRefreshingPrice
                                    ? props.t("refreshPriceLoading")
                                    : props.t("refreshPrice")}
                                </button>
                              ) : null}
                              {rowLink ? (
                                <a
                  ...
                  {!props.compactView && expanded ? (
                    <div className="qs-result-details" id={detailsId}>
                      <div>
                        ...
                        <p>
                          {r.distance_km_ground ? `${r.distance_km_ground} km` : "--"} -{" "}
                          {props.t("summaryRadius")} {props.radiusKm} km
                        </p>
                      </div>
                      <div>
                        <strong>{props.t("detailsWindow")}</strong>
                        <p>
                          {props.departAfter} - {props.departBefore}
                        </p>
                      </div>
                      <div>
                        ...
                        <p>
                          {props.t("scoreHint")} -{" "}
                          {r.ranking_score ? props.formatScore(r.ranking_score) : "--"}
                        </p>
                      </div>
                      <div>
                      ...
                      {r.legs && r.legs.length > 0 ? (
                        <div className="qs-legs">
                          ...
                          {r.legs.map((leg, legIdx) => {
                            ...
                            const legDurationMinutes = getLegDurationMinutes(
                              leg.dep_ts,
                              leg.arr_ts,
                            );
                            const legFlightTimeLabel =
                              legDurationMinutes !== null
                                ? props.formatMinutes(legDurationMinutes)
                                : null;
                            return (
                              <div key={`${rowId}-leg-${legIdx}`} className="qs-leg-row">
                                ...
                                <span>
                                  {props.t("weatherDepart")} {legDepartureClock || "--"}
                                </span>
                                <span>
                                  {props.t("weatherArrive")} {legArrivalClock || "--"}
                                  {legFlightTimeLabel
                                    ? ` · ${props.t("flightTime")} ${legFlightTimeLabel}`
                                    : ""}
                                </span>
                              </div>
                            );
