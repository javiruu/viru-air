...
import {
  useSaveCombination,
  type SaveResult,
} from "@/modules/quick-search/state/useSaveCombination";
...
import {
  ...
  isWeatherRangeSupported as isWeatherRangeSupportedCheck,
} from "@/modules/quick-search/weatherUtils";
import {
...
const RELAX_HIGHLIGHT_BY_ACTION: Record<
  ZeroResultRelaxAction,
  Exclude<SummaryHighlightKey, null>
> = {
...
function buildEmptyCalendarHintsCacheEntry(
  scopeMode: QuickSearchCalendarScopeMode,
): CalendarHintsCacheEntry {
  return {
    dayHintsByIso: {},
    scopeMode,
  };
...
export function QuickSearchView({ mode = "quick-search" }: { mode?: QuickSearchMode }) {
  ...
  const [calendarVisibleMonthReturn, setCalendarVisibleMonthReturn] =
    useState<string>(currentMonthIso);
  const [calendarHintsByKey, setCalendarHintsByKey] = useState<
    Record<string, CalendarHintsCacheEntry>
  >({});
  const [calendarHintsByKeyReturn, setCalendarHintsByKeyReturn] = useState<
    Record<string, CalendarHintsCacheEntry>
  >({});
  const [calendarHintsLoadingKey, setCalendarHintsLoadingKey] = useState<string | null>(null);
  const [calendarHintsLoadingKeyReturn, setCalendarHintsLoadingKeyReturn] = useState<string | null>(
    null,
  );
  ...
  const updateAdditionalAirport = useCallback(
    (side: "origin" | "destination", id: string, value: string) => {
      const updateEntries = (current: QuickSearchAdditionalAirport[]) =>
  ...
  const removeAdditionalAirport = useCallback((side: "origin" | "destination", id: string) => {
    const removeEntry = (current: QuickSearchAdditionalAirport[]) =>
  ...
  useEffect(() => {
    ...
    if (state.additionalOrigins.length > 0) {
      setAdditionalOrigins(
        state.additionalOrigins.map((value) => ({
    ...
    if (state.additionalDestinations.length > 0) {
      setAdditionalDestinations(
        state.additionalDestinations.map((value) => ({
    ...
    if (state.flexBefore) {
      setDaysBefore(state.flexBefore);
    }
    if (state.flexAfter) {
      setDaysAfter(state.flexAfter);
    }
  ...
  const [outboundFareProfile, setOutboundFareProfile] = useState(() =>
    createEmptyFareComparisonProfile(adults),
  );
  const [returnFareProfile, setReturnFareProfile] = useState(() =>
    createEmptyFareComparisonProfile(adults),
  );
  const pendingCombinationResultsRef = useRef<{
    outbound: SearchResult;
    return: SearchResult;
  } | null>(null);
  const submittedRouteSeedsRef = useRef<{
  ...
  const handleDualRelaxAction = useCallback(
    (action: ZeroResultRelaxAction, side: "outbound" | "return") => {
      ...
      const sideOriginSeeds =
        side === "outbound"
          ? submittedRouteSeedsRef.current.origin
          : submittedRouteSeedsRef.current.destination;
      const sideDestinationSeeds =
        side === "outbound"
          ? submittedRouteSeedsRef.current.destination
          : submittedRouteSeedsRef.current.origin;
  ...
  const pageWorkspaceHint = isRecommendations
    ? t("workspaceHintRecommendations")
    : t("workspaceHint");
  ...
  const formatScore = (value: number) =>
    formatNumber(value, { maximumFractionDigits: 2 }, localeTag);
  ...
  const fetchSeedAirports = useCallback(
    async (params: { q?: string; country_code?: string; limit?: number; offset?: number }) => {
  ...
  const fetchSeedCountriesFromSeeds = useCallback(async (): Promise<
    Array<{ code: string; name: string; airport_count: number }>
  > => {
  ...
  const fetchAutocompleteSuggestions = useCallback(
    async (value: string) => {
      ...
      for (const candidate of candidates) {
        ...
        const suggestions = buildAirportSuggestions(
          data.items || [],
          query,
          6,
          pref?.language || "en",
  ...
  const originValid = originCountryOnly
    ? originCountryOnly.airports.length > 0
    : originCode.length === 3 && airportsByIata.has(originCode);
  const destinationValid = destinationCountryOnly
    ? destinationCountryOnly.airports.length > 0
  ...
  const calendarHintGuidelineThresholds =
    useMemo<QuickSearchCalendarGuidelineThresholds | null>(() => {
      ...
      const safeMid =
        Number.isFinite(rawMid) && rawMid > safeLow ? rawMid : Math.max(safeLow + 1, 150);
  ...
  const calendarHintsActive = calendarHintsRequestKey
    ? calendarHintsByKey[calendarHintsRequestKey]
    : undefined;
  ...
  const calendarHintsActiveReturn = calendarHintsRequestKeyReturn
    ? calendarHintsByKeyReturn[calendarHintsRequestKeyReturn]
    : undefined;
  ...
  const findCountryByIataLocal = useCallback(
    (iata: string): CountryAirports | null => {
  ...
  const fetchWeather = (iata: string, start: string, end: string) =>
    fetchWeatherApi(iata, start, end, t);
  ...
  async function onSubmit(
    ...
    if (durationMax.trim() && parsedDurationMax === null)
      nextFieldErrors.duration_max = t("errorText");
    ...
    const ensureSeedCode = async (code: string) => {
      ...
      try {
        ...
        return (
          Array.isArray(response.items) && response.items.some((item) => item.iata === normalized)
        );
      } catch {
        return false;
      }
    ...
    const optionalOriginCodes = additionalOrigins
      .map((entry) => entry.value.trim().toUpperCase())
      .filter(Boolean);
    const optionalDestinationCodes = additionalDestinations
      .map((entry) => entry.value.trim().toUpperCase())
      .filter(Boolean);
    const optionalCodes = Array.from(
      new Set([...optionalOriginCodes, ...optionalDestinationCodes]),
    );
    const optionalValidity = await Promise.all(
      optionalCodes.map(async (code) => [code, await ensureSeedCode(code)] as const),
    );
    ...
    const destinationSeeds = buildRouteSeedList(
      destinationCountryOnly
        ? destinationCountryOnly.airports.map((item) => item.iata)
        : destinationCode,
      optionalDestinationCodes,
      knownAirportsForSubmit,
    );
    const validOptionalOriginCodes = Array.from(
      new Set(optionalOriginCodes.filter((code) => knownAirportsForSubmit.has(code))),
    );
    const validOptionalDestinationCodes = Array.from(
      new Set(optionalDestinationCodes.filter((code) => knownAirportsForSubmit.has(code))),
    );
    ...
    const destinationRequestValue =
      destinationSeeds.length === 1 ? (destinationSeeds[0] ?? "") : destinationSeeds;
    ...
    if (isDualMode) {
      ...
      parseIataList(excludeOriginInput).forEach((v: string) => {
        if (!dNextExcludeOrigins.includes(v)) dNextExcludeOrigins.push(v);
      });
      parseIataList(excludeDestinationInput).forEach((v: string) => {
        if (!dNextExcludeDestinations.includes(v)) dNextExcludeDestinations.push(v);
      });
    ...
    const destinationScopeCount = Array.isArray(payload.destination_iata)
      ? payload.destination_iata.length
      : 1;
    ...
    try {
      expectedQuerySignaturesRef.current =
        await buildQuickSearchExpectedSignatures(canonicalPayload);
      if (!isCurrentRequest()) return;
      void createSelectedDateWatches();
      if (!isPageChange) {
        setIsLoading(true);
      ...
      const weatherRangeSupported =
        range.length > 0 && isWeatherRangeSupportedCheck(range[0], range[range.length - 1]);
      if (
        !isPageChange &&
        !weatherRangeSupported &&
        (originWeatherIata || destinationWeatherIata)
      ) {
        setWeatherMessage(t("weatherUnavailableRange"));
      }

      if (!isPageChange) {
        const originWeatherPromise =
          weatherRangeSupported && originWeatherIata
            ? fetchWeather(originWeatherIata, range[0], range[range.length - 1])
            : Promise.resolve(null);
        const destinationWeatherPromise =
          weatherRangeSupported && destinationWeatherIata
            ? fetchWeather(destinationWeatherIata, range[0], range[range.length - 1])
            : Promise.resolve(null);
        void Promise.allSettled([originWeatherPromise, destinationWeatherPromise]).then(
      ...
      if (searchResult.ok) {
        ...
        if (
          typeof data.meta?.total_candidates === "number" &&
          Number.isFinite(data.meta.total_candidates)
        ) {
          ...
          setProviderSearchStatuses(
            rawProviderStatuses.map(
              (p: {
                ...
                if (p.degraded || (p.errors && p.errors > 0) || p.status === "failed")
                  badgeStatus = "error";
                else if (p.status === "ok" || (p.results_count && p.results_count > 0))
                  badgeStatus = "found";
                const label = p.id
        ...
        void trackUxEvent("quick_search_executed", {
        ...
        if (status === 429) {
          ...
          setSearchState("error");
          setSearchError(
            Object.keys(validationErrors).length > 0 ? t("errorText") : t("searchFailed"),
          );
        }
        setProviderSearchStatuses([]);
        setHasSearched(true);
  ...
  function canRefreshPrice(result: SearchResult) {
    ...
    return (
      Boolean(result.origin && result.destination && result.travel_date) &&
  ...
  const getResultWatchlistHref = useCallback(
  ...
  const viewResultInWatchlist = useCallback(
    (result: SearchResult) => {
  ...
  function openPicker(which: "origin" | "destination") {
    ...
    const country =
      (which === "origin" ? originCountryOnly : destinationCountryOnly) ||
  ...
  const loadCountryAirports = useCallback(
  ...
  const rememberAirportSelection = useCallback(
    (field: QuickSearchAutocompleteField, iata: string) => {
      const storage = typeof window !== "undefined" ? window.localStorage : null;
      if (field === "origin") {
        const next = writeRecentAirports(
          rememberRecentAirport(originRecentAirports, iata),
          storage,
          undefined,
          RECENT_AIRPORTS_ORIGIN_KEY,
        );
        ...
        const next = writeRecentAirports(
          rememberRecentAirport(destinationRecentAirports, iata),
          storage,
          undefined,
          RECENT_AIRPORTS_DESTINATION_KEY,
        );
  ...
  const removeRecentAirportSelection = useCallback(
    (field: QuickSearchAutocompleteField, iata: string) => {
      const storage = typeof window !== "undefined" ? window.localStorage : null;
      if (field === "origin") {
        const next = writeRecentAirports(
          forgetRecentAirport(originRecentAirports, iata),
          storage,
          undefined,
          RECENT_AIRPORTS_ORIGIN_KEY,
        );
        ...
        const next = writeRecentAirports(
          forgetRecentAirport(destinationRecentAirports, iata),
          storage,
          undefined,
          RECENT_AIRPORTS_DESTINATION_KEY,
        );
  ...
  const selectCountryOnly = useCallback(
    async (country: CountryAirports | null) => {
  ...
  useEffect(() => {
    if (!activePicker && !isFiltersOpen && !copyModalOpen && !isExplainOpen && !openRowMenuId)
      return;
  ...
  function selectAutocompleteSuggestion(
    field: QuickSearchAutocompleteField,
    iata: string,
  ...
  const removeChip = useCallback(
    (value: string, current: string[], setCurrent: (next: string[]) => void) => {
      setCurrent(current.filter((item) => item !== value));
    },
    [],
  );

  const updateRadiusKm = useCallback(
    (value: number) => {
  ...
  }, [excludeOriginInput, excludeOrigins, setExcludeOriginInput, setExcludeOrigins, addChip]);
  ...
  function mapFieldValidationMessage(field: QuickSearchField, message: string): string {
    ...
    if (field === "travel_date") {
      if (normalized.includes("required") || normalized.includes("missing"))
        return t("selectOutbound");
      return message;
    }
    if (field === "origin_iata") {
      if (normalized.includes("required") || normalized.includes("missing"))
        return t("originRequired");
      if (normalized.includes("iata") || normalized.includes("pattern") || normalized.includes("3"))
        return t("iataInvalid");
      return message;
    }
    if (normalized.includes("required") || normalized.includes("missing"))
  ...
  const resultKey = useCallback((result: SearchResult, fallback: number) => {
    return (
      result.result_id || `${result.origin}-${result.destination}-${result.travel_date}-${fallback}`
    );
  }, []);
    ...
    <div className="airport-modal-overlay" onClick={closePicker}>
      ...
      >
        ...
        <div className="qs-airport-modal__body">
          <div
            className="airport-modal-left qs-airport-modal__countries"
            role="region"
            aria-label={
              activePicker === "origin" ? t("modalOriginCountry") : t("modalDestinationCountry")
            }
          >
            <div
              className="qs-airport-modal__header"
            ...
            >
              ...
              <div
                className="airport-search qs-airport-modal__search"
                role="search"
                style={{ padding: 0, border: "none" }}
              >
                <svg
                  className="qs-airport-modal__search-icon"
                  width="16"
                  height="16"
                  viewBox="0 0 16 16"
                  fill="none"
                  aria-hidden="true"
                  style={{ left: "12px", top: "14px" }}
                >
            ...
            <div
              className="airport-country-grid"
              role="listbox"
              aria-label={
                activePicker === "origin" ? t("modalOriginCountry") : t("modalDestinationCountry")
              }
          ...
          <div className="airport-modal-right qs-airport-modal__airports" aria-live="polite">
            ...
            {selectedCountry && !additionalPickerTarget ? (
              <div className="qs-airport-modal__country-action">
                <button
                  type="button"
                  className="btn-secondary btn-compact qs-airport-modal__use-country"
                  onClick={() => void selectCountryOnly(selectedCountry)}
                >
            ...
            {(activePicker === "origin" ? originRecentAirports : destinationRecentAirports).length >
            0 ? (
                ...
                <div className="airport-recent-grid">
                  {(activePicker === "origin"
                    ...
                    <button
                      key={`recent-${iata}`}
                      type="button"
                      className="qs-airport-modal__recent-chip"
            ...
            <div className="airport-list">
              {filteredCountryAirports.map((airport) => (
                <button
                  key={airport.iata}
                  type="button"
                  className="qs-airport-modal__airport-item"
                  onClick={() => selectAirport(airport.iata)}
                >
                  {renderFlag(selectedCountry?.code || null)}
                  <span className="qs-airport-modal__airport-name">
  ...
  const originVisibleSuggestions = origin.trim()
    ? originSuggestions
    : originRecentAirportSuggestions;
  const destinationVisibleSuggestions = destination.trim()
    ? destinationSuggestions
    : destinationRecentAirportSuggestions;
  ...
  const activeSuggestionId =
    ...
    activeAutocompleteIndex < activeSuggestions.length
  ...
  const returnDateInvalid = Boolean(
    isReturn && travelDate && returnDate && returnDate < travelDate,
  );
  ...
  useEffect(() => {
    if (saveCombination.status === "saved") {
      ...
      if (savedResults?.outbound)
        markAsSaved(savedResults.outbound, saveCombination.outboundWatchId);
      ...
      notify({
        ...
        onAction: () =>
          navigateToWatchlistWithContext(
            ...
            destination,
            travelDate,
            saveCombination.outboundWatchId,
          ),
        durationMs: 3200,
      });
  ...
  const currentCriteriaSignature = useMemo(
    () =>
      buildCriteriaSignature({
        origin,
        destination,
  ...
  const pendingSearchChanges = Boolean(
    hasSearched &&
      appliedCriteriaSignature &&
      currentCriteriaSignature !== appliedCriteriaSignature,
  );
  ...
  const summaryDuration =
    summaryDurationValue !== null && Number.isFinite(summaryDurationValue)
  ...
  const compactSummaryChips = useMemo<QuickSearchSummaryChip[]>(() => {
    ...
    if (includeNearbyDestinations) {
      chips.push({
        ...
        label: t("summaryNearbyDestination"),
        tone: "search",
      });
    }
    if (radiusActive) {
    ...
    if (travelDate) {
      chips.push({
        id: "date",
        label: `${t("summaryExactDateChip")}: ${summaryDate}`,
        tone: "route",
      });
    }
    if (daysBefore > 0 || daysAfter > 0) {
  ...
  const totalFlightsForLoader = Math.max(
    0,
    loaderResolvedTotalFlights ?? loaderPlannedTotalFlights,
  );
  const currentFlightsForLoader =
    totalFlightsForLoader > 0
      ? Math.min(totalFlightsForLoader, Math.round((progressPercent / 100) * totalFlightsForLoader))
      : 0;
  const loadingTotalText = t("loadingTotalFlights").replace(
    "{count}",
    formatNumber(totalFlightsForLoader, {}, localeTag),
  );
  ...
  const easedProgressRatio = progressRatio ** 2.2;
  ...
  const loadingSubcheckRoutes = useMemo(() => {
    ...
    const destinationPool = buildPool(
      destination,
      destinationCountryOnly,
      includeNearbyDestinations,
    );
  ...
  const loadingSubcheckActiveIndex =
    loadingPhase === "requesting"
      ? progressPercent >= 55
        ? 1
        : 0
    ...
    return Array.from({ length: 3 }).map((_, idx) => {
      ...
      const label = route
        ...
        : t("loadingSubcheckCombo")
            ...
            .replace("{total}", "3");
      const status: QuickSearchLoadingSubcheckStatus =
        loadingSubcheckActiveIndex >= 3
          ? "done"
  ...
  const destinationCountry = resolveInputCountryCode(
    destinationCountryOnly?.code,
    destinationSelectedCountryCode,
  );
  ...
  const _resultsSignature = useMemo(() => {
  ...
  const totalResults = Math.max(
    0,
    Number(backendPagination?.total_results || visibleResults.length),
  );
  const canExportQuickSearch =
    Boolean(lastQuickSearchPayloadRef.current) &&
    searchState === "success" &&
    totalResults > 0 &&
    !isLoading &&
  ...
  const providerPartialInlineMessages = QUICK_SEARCH_PROVIDER_PARTIAL_INLINE_WARNING_CODES.map(
    (code) => tWarn(code),
  );
  ...
  const providerPartialInlineText =
    providerPartialInlineNotice ??
    (providerPartialWarnings.length > 0
      ? providerPartialWarnings
          .map((group) => `${group.message}${group.count > 1 ? ` (${group.count})` : ""}`)
          .join(" ")
      : "");
  ...
  useEffect(() => {
    ...
    globalFreshness.observedAt,
    globalFreshness.status,
    hasSearched,
    jobId,
    freshnessShownKeyRef,
  ]);
  ...
  const relaxPreviewChanges = useMemo(() => {
    ...
    if (strictFilters) {
      rows.push({
        id: "strict",
        label: t("strictMode"),
        before: t("summaryStrictOn"),
        after: t("summaryStrictOff"),
      });
    }
    if (priceMin) {
  ...
  const undoZeroResultRelaxAction = useCallback(
    (requestedAction?: ZeroResultRelaxAction) => {
      ...
      if (undoPayload.action === "disable_strict") {
        ...
        undoPayload.action === "open_date_flex" ||
        undoPayload.action === "try_plus_1_day"
      ) {
  ...
  const onZeroResultRelaxAction = useCallback(
    (action: ZeroResultRelaxAction) => {
  ...
  const selectedResult = useMemo(() => {
    ...
    return visibleResults.find((item, idx) => resultKey(item, idx) === selectedResultId) || null;
  }, [visibleResults, selectedResultId, resultKey]);
  ...
  const panelSearchState = hasFinalEmptyState
    ? "empty"
    : hasFinalResults
      ? "success"
      : visualSearchState;
  const resultsStageTitle = isVisualLoading
    ? t("loadingTitle")
    : hasFinalEmptyState
      ? emptyStateMainTitle
      : hasFinalResults
  ...
  const removeExcludeOriginChip = useCallback(
    (iata: string) => {
      removeChip(iata, excludeOrigins, setExcludeOrigins);
    },
    [excludeOrigins, setExcludeOrigins, removeChip],
  );

  const removeExcludeDestinationChip = useCallback(
    (iata: string) => {
      removeChip(iata, excludeDestinations, setExcludeDestinations);
    },
    [excludeDestinations, setExcludeDestinations, removeChip],
  );
    ...
    <main className="shell quick-search-shell" id="main-content">
        ...
        <div className="qs-command-stage__top">
          <div className="panel panel-soft qs-command-stage__intro">
            ...
            {quickSearchHint.visible ? (
              <section
                className="notice notice-compact notice-info qs-hero-hint"
                role="status"
                aria-live="polite"
              >
                <div>
                ...
                <div className="notice-actions">
                  <button
                    ...
                    onClick={quickSearchHint.dismiss}
                  >
        ...
        >
            ...
            <div className="qs-route-card">
              <label className="qs-label">
                <span>
                  ...
                  <span
                    className="qs-tip"
                    data-tip={t("originTip")}
                    tabIndex={0}
                    role="note"
                    aria-label={t("originTip")}
                  >
                    <svg viewBox="0 0 24 24" role="img" aria-hidden="true">
                      <circle
                        cx="12"
                      ...
                      />
                      <path
                        d="M12 16.2v.2M9.8 9.2a2.2 2.2 0 1 1 3.3 1.9c-.8.5-1.1.9-1.1 1.8"
                        fill="none"
                        stroke="currentColor"
                        strokeWidth="1.8"
                        strokeLinecap="round"
                ...
                <div className="qs-input-wrap">
                  ...
                  <input
                    ...
                    onKeyDown={(event) => {
                      ...
                      if (event.key === "Enter") {
                        const selected =
                          activeAutocompleteField === "origin" && activeAutocompleteIndex >= 0
                    ...
                    placeholder={
                      originCountryOnly ? originCountryOnly.name : randomOriginPlaceholder
                    }
                    aria-invalid={
                      (originTouched && !originValid) || Boolean(fieldErrors.origin_iata)
                    }
                    aria-describedby="origin-help"
                    aria-expanded={activeAutocompleteField === "origin"}
                    aria-controls="origin-suggestions"
                    aria-activedescendant={
                      activeAutocompleteField === "origin" ? activeSuggestionId : undefined
                    }
                  />
                  <button
                    type="button"
                  ...
                  {activeAutocompleteField === "origin" && originVisibleSuggestions.length > 0 ? (
                    <ul
                      className={
                        !origin.trim()
                          ? "qs-autocomplete qs-autocomplete-recents"
                          : "qs-autocomplete"
                      }
                    ...
                    >
                      {!origin.trim() ? (
                        <li className="qs-autocomplete-group-label">
                          {t("recentAutocompleteLabel")}
                        </li>
                      ) : null}
                          ...
                          >
                            <div className={!origin.trim() ? "qs-recent-row" : undefined}>
                              <button
                                ...
                                type="button"
                                className={
                                  isActive ? "qs-autocomplete-item active" : "qs-autocomplete-item"
                                }
                                onMouseDown={(event) => event.preventDefault()}
                                onClick={() =>
                                  selectAutocompleteSuggestion("origin", suggestion.iata)
                                }
                              >
                              ...
                              {!origin.trim() ? (
                                <button
                                  ...
                                  aria-label={t("removeRecentAirportAria").replace(
                                    "{iata}",
                                    suggestion.iata,
                                  )}
                                  onMouseDown={(event) => event.preventDefault()}
                                  onClick={() =>
                                    removeRecentAirportSelection("origin", suggestion.iata)
                                  }
                                >
                                  <svg viewBox="0 0 24 24" role="img" aria-hidden="true">
                                    <path
                ...
                {originCountryOnly ? (
                  <div className="qs-country-row">
                    <span className="qs-chip">
                      {t("countryOnlySelected").replace("{country}", originCountryOnly.name)}
                    </span>
                    <button
                      type="button"
                      className="btn-ghost btn-compact"
                      onClick={() => setOriginCountryOnly(null)}
                    >
                      {t("countryOnlyClear")}
                    </button>
                  </div>
                ...
                {(originTouched && !originValid) || fieldErrors.origin_iata ? (
                  <small className="qs-error">
                    {fieldErrors.origin_iata ||
                      (origin.trim() ? t("iataInvalid") : t("originRequired"))}
                  </small>
                ) : null}
              </label>
            ...
            <div className="qs-route-card">
              <label className="qs-label">
                <span>
                  ...
                  <span
                    className="qs-tip"
                    data-tip={t("destinationTip")}
                    tabIndex={0}
                    role="note"
                    aria-label={t("destinationTip")}
                  >
                    <svg viewBox="0 0 24 24" role="img" aria-hidden="true">
                      <circle
                        cx="12"
                ...
                <div className="qs-input-wrap">
                  ...
                  <input
                    ...
                    onKeyDown={(event) => {
                      ...
                      if (event.key === "Enter") {
                        const selected =
                          activeAutocompleteField === "destination" && activeAutocompleteIndex >= 0
                    ...
                    placeholder={
                      destinationCountryOnly
                        ? destinationCountryOnly.name
                        : randomDestinationPlaceholder
                    }
                    aria-invalid={
                      (destinationTouched && !destinationValid) ||
                      Boolean(fieldErrors.destination_iata)
                    }
                    aria-describedby="destination-help"
                    aria-expanded={activeAutocompleteField === "destination"}
                    aria-controls="destination-suggestions"
                    aria-activedescendant={
                      activeAutocompleteField === "destination" ? activeSuggestionId : undefined
                    }
                  />
                  <button
                    type="button"
                  ...
                  {activeAutocompleteField === "destination" &&
                  destinationVisibleSuggestions.length > 0 ? (
                    <ul
                      className={
                        !destination.trim()
                          ? "qs-autocomplete qs-autocomplete-recents"
                    ...
                    >
                      ...
                      {destinationVisibleSuggestions.map((suggestion, index) => {
                        ...
                        return (
                          <li
                            key={`destination-${suggestion.iata}`}
                            role="option"
                            aria-selected={isActive}
                          >
                            <div className={!destination.trim() ? "qs-recent-row" : undefined}>
                              <button
                                ...
                                onMouseDown={(event) => event.preventDefault()}
                                onClick={() =>
                                  selectAutocompleteSuggestion("destination", suggestion.iata)
                                }
                              >
                                <strong>{suggestion.iata}</strong>
                                <span>{suggestion.name}</span>
                              ...
                              {!destination.trim() ? (
                                <button
                                  ...
                                  onMouseDown={(event) => event.preventDefault()}
                                  onClick={() =>
                                    removeRecentAirportSelection("destination", suggestion.iata)
                                  }
                                >
                                  <svg viewBox="0 0 24 24" role="img" aria-hidden="true">
                                    <path
                ...
                {destinationCountryOnly ? (
                  <div className="qs-country-row">
                    <span className="qs-chip">
                      {t("countryOnlySelected").replace("{country}", destinationCountryOnly.name)}
                    </span>
                    <button
                      type="button"
                      className="btn-ghost btn-compact"
                      onClick={() => setDestinationCountryOnly(null)}
                    >
                      {t("countryOnlyClear")}
                    </button>
                  </div>
                ...
                {(destinationTouched && !destinationValid) || fieldErrors.destination_iata ? (
                  <small className="qs-error">
                    {fieldErrors.destination_iata ||
                      (destination.trim() ? t("iataInvalid") : t("destinationRequired"))}
                  </small>
                ) : null}
              </label>
              <PopularDestinationsChips
          ...
          <div className="qs-passengers">
            ...
            <div
              ...
              aria-label={t("passengersStepperAria").replace("{count}", String(adults))}
            >
              <button
                type="button"
            ...
            <small className="qs-search-hint qs-passengers-note">
              {t("passengersBaseFareHint")}
            </small>
          </div>
          ...
          <div className={`qs-date-grid${isReturn ? " has-return" : ""}`}>
            <label className="date-field qs-label">
              ...
              <QuickSearchDatePicker
                ...
                showCountryEstimateBadge={
                  canRequestCalendarHints && hasCountryScopeForCalendarHints
                }
            ...
            {isReturn ? (
              <label className="date-field qs-label">
                ...
                <QuickSearchDatePicker
                  ...
                  invalid={
                    (returnDateTouched && !returnDate) ||
                    Boolean(fieldErrors.return_date) ||
                    returnDateInvalid
                  }
                  ...
                  onChange={(value) => {
                    ...
                    setFieldErrors((prev) => ({
                      ...prev,
                      return_date:
                        value && travelDate && value < travelDate ? t("returnBefore") : undefined,
                    }));
                  }}
                />
                {(returnDateTouched && !returnDate) ||
                ...
                returnDateInvalid ? (
                  <small className="qs-error">
                    {fieldErrors.return_date ||
                      (returnDateInvalid ? t("returnBefore") : t("selectReturn"))}
                  </small>
                ) : tripType === "round_trip_incomplete" ? (
      ...
      {showResultsWorkspace && !isDualMode ? (
        <QuickSearchResultsWorkspace>
            ...
            <div className="qs-workspace-grid">
              <section className="panel panel-soft qs-results-panel">
                {showResultsStagehead ? (
                  <div className="qs-results-stagehead">
                    <div className="qs-results-stagehead__status">
                      ...
                      <p>
                        {hasSearched ? `${t("orderedBy")} ${sortLabel[sortBy]}` : pageWorkspaceHint}
                      </p>
                    </div>
                    {pendingSearchChanges ? (
                      <span className="qs-results-stagehead__pending">
                ...
                {showResultsToolbar ? (
                  <div className="qs-results-toolbar" ref={resultsToolbarRef} tabIndex={-1}>
                    <div className="qs-results-summary">
                      ...
                      {hasSearched ? (
                        <span className="muted">
                          {" "}
                          - {t("orderedBy")} {sortLabel[sortBy]}
                        </span>
                      ) : null}
                      {searchMeta?.truncated ? (
                        <span className="chip chip-warn">{t("truncated")}</span>
                      ) : null}
                      {hasSearched ? (
                        <span className="qs-freshness-global">
                          {!globalFreshness.isUnavailable ? (
                            <span>{globalFreshness.label}</span>
                          ) : (
                    ...
                    <div className="qs-results-controls">
                      ...
                      >
                        <summary
                          className={`qs-explain-trigger qs-results-explain-chip qs-how-order__summary ${showDegradedState ? "qs-degraded-chip" : ""}`}
                          role="button"
                          aria-label={t("explainTitle")}
                        ...
                        <div className="panel panel-soft qs-explain-panel qs-how-order__panel">
                          ...
                          {selectedResult ? (
                            <div className="qs-explain-selected">
                              <strong>
                                {selectedResult.origin} {" â†’ "} {selectedResult.destination}
                              </strong>
                              <span>
                                {t("score")}:{" "}
                                {selectedResult.ranking_score
                                  ? formatScore(selectedResult.ranking_score)
                                  : "--"}
                              </span>
                      ...
                      <button
                        ...
                        aria-label={t("ariaFiltersToggle").replace(
                          "{count}",
                          String(activeChips.length),
                        )}
                        ...
                        onClick={() => {
                          if (isFiltersOpen) {
                            ...
                            trackEvent("quicksearch_filters_opened", {
                              active_filters: activeChips.length,
                            });
                ...
                {relaxPreviewOpen ? (
                  <section className="panel panel-soft section-gap-sm" aria-live="polite">
                    ...
                    <div className="qs-results-controls">
                      <button type="button" className="btn-search" onClick={applyRelaxPreview}>
                        {t("relaxPreviewConfirm")}
                      </button>
                      <button type="button" className="btn-ghost" onClick={cancelRelaxPreview}>
                        {t("relaxPreviewCancel")}
                      </button>
                    </div>
                  </section>
                ) : null}
                ...
                {providerTotalWarnings.length > 0 && visibleResults.length === 0 ? (
                  <div
                    className="notice notice-error section-gap-sm"
                    role="status"
                    aria-live="polite"
                  >
                    {providerTotalWarnings
                      .map(
                        (group) => `${group.message}${group.count > 1 ? ` (${group.count})` : ""}`,
                      )
                      .join(" ")}
                  </div>
                ) : null}
                {providerPartialInlineText && showResultsList ? (
                  <div
                    className="notice notice-info section-gap-sm"
                    role="status"
                    aria-live="polite"
                  >
                      ...
                      >
                        <div className="qs-pagination-stats">
                          {t("paginationShowing")
                            .replace(
                              "{start}",
                              String(totalResults === 0 ? 0 : (activePage - 1) * pageSize + 1),
                            )
                            .replace("{end}", String(Math.min(activePage * pageSize, totalResults)))
                            .replace("{total}", String(totalResults))}
                          {isPageChanging ? (
                            <span className="qs-pagination-live">Preparando pagina</span>
                          ) : null}
                        </div>
              ...
              <aside className="qs-context-rail">
                {pendingActionVisibility.contextRailNotice ? (
                  <div
                    className="notice notice-warning qs-pending-changes"
                    role="status"
                    aria-live="polite"
                  >
                ...
                >
                  ...
                  <div className="qs-info-body">
                    <section className="qs-info-tier qs-info-tier-status" aria-live="polite">
                      ...
                      <strong>
                        {showDegradedState ? t("degradedBadge") : t("searchReadyTitle")}
                      </strong>
                      <p>
                    ...
                    {sourcesSummary.entries.length > 0 ? (
                      <details
                        ...
                        onToggle={(event) => {
                          ...
                          trackEvent("quicksearch_sources_detail_opened", {
                            sources_count: sourcesSummary.entries.length,
                          });
                        }}
                      >
                        <summary className="qs-sources-trigger" aria-label={t("sourcesDetailAria")}>
                          <span>
                            {t("sourcesLabel")}:{" "}
                            {sourcesSummary.preview || `${t("sourceUnknown")} (0)`}
                          </span>
          ...
          {copyModalOpen ? (
            <div className="airport-modal-overlay" onClick={() => setCopyModalOpen(false)}>
              <section
                ...
                role="dialog"
                aria-modal="true"
                aria-label={t("deepLinkModalTitle")}
                onClick={(event) => event.stopPropagation()}
              >
          ...
          {deepLinkError ? (
            <div className="notice notice-error section-gap-sm">{deepLinkError}</div>
          ) : null}
          {message ? (
            <div
              className={`notice section-gap ${messageType === "success" ? "notice-success" : "notice-error"}`}
            >
              {message}
            </div>
          ) : null}
      ...
      {isDualMode && (outboundSide.searchState !== "idle" || returnSide.searchState !== "idle") ? (
        <QuickSearchDualWorkspace ariaLabel="Round-trip results" hoveredSide={dualHoverSide}>
          ...
          <QuickSearchSidePanel
            side="outbound"
            origin={
              originCountryOnly != null ? (originCountryOnly as CountryAirports).name : origin
            }
            destination={
              destinationCountryOnly != null
                ? (destinationCountryOnly as CountryAirports).name
                : destination
            }
            dateLabel={formatShortDate(travelDate)}
            headerLabel={t("sideOutboundLabel")}
            resultCount={
              outboundSide.searchState === "success" ? outboundPanelState.visibleResults.length : 0
            }
            currentPage={
              outboundSide.searchState === "success" ? outboundSide.currentPage : undefined
            }
            ...
            onPageChange={
              outboundSide.searchState === "success"
                ? (page: number) => outboundSide.goToPage(page)
                : undefined
            }
            locale={locale}
          ...
          >
            ...
            {outboundSide.searchState === "loading" ||
            outboundSide.searchState === "error" ||
            outboundSide.searchState === "empty" ||
            outboundSide.searchState === "rate" ||
            (outboundSide.searchState === "success" &&
              outboundPanelState.visibleResults.length === 0) ? (
              <QuickSearchStatePanels
                searchState={
                  outboundSide.searchState === "success" ? "empty" : outboundSide.searchState
                }
                ...
                onRelaxAction={(action) => {
                  void handleDualRelaxAction(action, "outbound");
                }}
          ...
          <QuickSearchSidePanel
            ...
            resultCount={
              returnSide.searchState === "success" ? returnPanelState.visibleResults.length : 0
            }
            ...
            onPageChange={
              returnSide.searchState === "success"
                ? (page: number) => returnSide.goToPage(page)
                : undefined
            }
            locale={locale}
          ...
          >
            ...
            {returnSide.searchState === "loading" ||
            returnSide.searchState === "error" ||
            returnSide.searchState === "empty" ||
            returnSide.searchState === "rate" ||
            (returnSide.searchState === "success" &&
              returnPanelState.visibleResults.length === 0) ? (
              <QuickSearchStatePanels
                searchState={
                  returnSide.searchState === "success" ? "empty" : returnSide.searchState
                }
                ...
                onRelaxAction={(action) => {
                  void handleDualRelaxAction(action, "return");
                }}
              ...
              <QuickSearchResultsList
                ...
                openRowMenuId={null}
                deeplinkUrl={
                  returnSide.deepLink?.url ||
                  returnSide.deepLink?.fallback_url ||
                  buildReturnFallbackUrl()
                }
                origin={destination}
                destination={origin}
                radiusKm={radiusKm}
          ...
          <QuickSearchCombinedBanner
            combinedPrice={(() => {
              const ob = outboundSide.selectedResultId
                ? outboundPanelState.visibleResults.find(
                    (r, i) => resultKey(r, i) === outboundSide.selectedResultId,
                  )
                : outboundPanelState.visibleResults[0];
              const rb = returnSide.selectedResultId
                ? returnPanelState.visibleResults.find(
                    (r, i) => resultKey(r, i) === returnSide.selectedResultId,
                  )
            ...
            onSave={() => {
              const ob = findCombinationResult(
                outboundPanelState.visibleResults,
                outboundSide.selectedResultId,
              );
              const rb = findCombinationResult(
                returnPanelState.visibleResults,
                returnSide.selectedResultId,
              );
              if (!ob || !rb) {
