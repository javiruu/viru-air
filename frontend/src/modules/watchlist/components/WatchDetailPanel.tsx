...
import {
  ...
  hasPriceSummaryData,
} from "@/modules/watchlist/summary";
...
export function WatchDetailPanel({
  ...
  useEffect(() => {
    setFareProfile(
      selectedWatch?.fare_profile ??
        currentDetail?.fare_profile ??
        createEmptyFareComparisonProfile(1),
    );
    setFareProfileSaved(false);
    setFareProfileSaveFailed(false);
  }, [currentDetail?.fare_profile, selectedWatch?.fare_profile]);
  ...
  const latestProvider = latestSnapshot
    ? resolveProviderPresentation(latestSnapshot.provider, t("watchlist.providerCoverage.unknown"))
    : null;
  const currency = latestSnapshot?.raw_currency ?? "EUR";
  const hasSnapshotPrice =
    latestSnapshot && latestSnapshot.raw_price != null && latestSnapshot.raw_price >= 0;
  const currentPriceValue = hasSnapshotPrice
    ? formatCurrency(latestSnapshot.raw_price, currency, localeTag)
    : "--";
  ...
  const minPriceValue =
    summaryData?.min_price == null
      ? "--"
      : formatCurrency(summaryData.min_price, currency, localeTag);
  const deltaFromMin =
    latestSnapshot && summaryData?.min_price != null
      ? latestSnapshot.raw_price - summaryData.min_price
      : null;
  const deltaFromMinValue =
    deltaFromMin == null ? "--" : formatCurrency(deltaFromMin, currency, localeTag);

  const trendText =
    summaryData?.delta_pct == null
    ...
    <section className="panel panel-soft section-gap watch-detail-panel">
      ...
      <div key={focus.id} className="watch-detail-selection-transition">
        <div className="watch-detail-hero">
          ...
          <span className={`status-pill watch-detail-hero-status ${status.tone}`}>
            {status.label}
          </span>
        </div>
          ...
          <div className="watch-detail-metrics">
            ...
            {latestSnapshot ? (
              <div className="watch-detail-metric">
                ...
                {freshness.observationNote ? (
                  <small className="panel-note">{freshness.observationNote}</small>
                ) : null}
              </div>
            ) : null}
          </div>
            ...
            <div className="fare-comparison-summary" aria-live="polite">
              ...
              <button
                ...
                onClick={async () => {
                  ...
                  setFareProfileSaveFailed(false);
                  try {
                    const profileToSave = attachFareAirline(fareProfile, latestSnapshot?.provider);
              ...
              {fareProfileSaved ? (
                <small>{t("watchlist.detail.fareComparison.saved")}</small>
              ) : null}
        ...
        <div className="watch-detail-block">
          ...
          <div className="watch-detail-operational">
            {latestSnapshot ? (
              <>
                <span className="watch-detail-operational-item tabular-nums">
                  {t("watchlist.detail.latestSnapshot")}{" "}
                  {safeDateTime(latestSnapshot.captured_at_utc, localeTag)}
                </span>
                {latestProvider ? (
                  <span
                    className={`watch-detail-operational-item watch-provider-chip watch-provider-chip--${latestProvider.id}`}
                  >
                    {t("watchlist.providerCoverage.detailSource", {
                      provider: latestProvider.label,
                    })}
                  </span>
                ) : null}
              </>
            ...
            <span className="watch-detail-operational-item tabular-nums">
              {t("watchlist.summary.count")} {summaryData ? summaryData.count : "--"}
            </span>
            <span className="watch-detail-operational-item tabular-nums">
              {t("watchlist.summary.delta")}{" "}
              {summaryData?.delta_pct == null
                ? "--"
                : formatPercent(summaryData.delta_pct, localeTag)}
            </span>
            <span className="watch-detail-operational-item">
              {t("watchlist.detail.operational.trend")} {trendText}
            </span>
          </div>
        </div>
        ...
        {summaryData ? (
          <div
            className="notice notice-info notice-compact history-confidence-notice watch-detail-confidence"
            role="status"
            aria-live="polite"
          >
            <strong>
              {confidence.titleKey
                ? t(confidence.titleKey)
                : t("watchlist.detail.interpretation.title")}
            </strong>
        ...
        {isLoading && !currentDetail ? (
          <BoneyardLoad
            name="watch-detail-history-load"
            className="history-summary history-summary--kpis"
            ariaLabel={t("watchlist.smartList.loadingAria")}
          >
        ...
        <div className="alert-actions watch-detail-actions">
          {focus.community_pricing.eligible ? null : focus.status === "paused" ? (
            <button
              ...
              type="button"
              onClick={() => onResumeWatch(focus.id)}
            >
