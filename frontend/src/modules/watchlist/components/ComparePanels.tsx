...
function volatilityLabel(
  value: "low" | "medium" | "high" | "insufficient_data",
  t: (key: string) => string,
): string {
...
export function ComparePanels({
  ...
  const [hoveredPoint, setHoveredPoint] = useState<
    (CompareChartPoint & { label: string; color: string; travelDate: string }) | null
  >(null);
  ...
  const compareBadgesFromResponse = useMemo(() => {
    ...
    const stable = watches
      ...
      .reduce<(typeof watches)[number] | null>((acc, item) => {
        ...
        if (!acc) return item;
        const accRank =
          acc.volatility_hint === "low" ? 0 : acc.volatility_hint === "medium" ? 1 : 2;
        return rank < accRank ? item : acc;
      }, null);
    const freshestPoint = (compareResponse?.points ?? []).reduce<{
      watchId: string;
      ts: number;
    } | null>((acc, point) => {
    ...
    const series: CompareChartSerie[] = prepared.map((serie) => {
      const points = serie.points.map((point) => {
        ...
        const y =
          CHART_HEIGHT -
          CHART_PAD.bottom -
          yRatio * (CHART_HEIGHT - CHART_PAD.top - CHART_PAD.bottom);
        return {
          date: point.date,
          x,
          y,
  ...
  return (
    <section
      className={`panel compare-panel section-gap${compareIds.length < 2 ? " compare-empty" : ""}`}
    >
      <div className="panel-header">
        ...
        <span className="compare-count">
          {compareIds.length}/4 {t("watchlist.compare.selected")}
        </span>
      </div>
      ...
      <div className="compare-panel-body">
        ...
        {hasCompareError ? (
          <div className="notice notice-error notice-compact">
            {t("watchlist.compare.errorInline")}
          </div>
        ) : null}
        {compareResponse?.currency_mode === "mixed" ? (
          <div className="notice notice-info notice-compact">
            {t("watchlist.compare.mixedCurrencyWarning")}
          </div>
        ) : null}
        ...
        {selectedCount === 0 ? (
          ...
          <BoneyardLoad
            name="watchlist-compare-load"
            className="boneyard-list"
            ariaLabel={t("watchlist.compare.loading")}
          >
          ...
          <div className="compare-multi-container">
            {compareChartSeries.hasData && (
              <div
                className="compare-chart compare-chart--global"
                data-testid="compare-master-chart"
                ...
                <div className="compare-chart-frame" style={{ position: "relative" }}>
                  ...
                  >
                    {[0, 0.33, 0.66, 1].map((ratio) => {
                      const y =
                        CHART_PAD.top + (CHART_HEIGHT - CHART_PAD.top - CHART_PAD.bottom) * ratio;
                      return (
                        <line
                          key={`y-grid-${ratio}`}
                          x1={CHART_PAD.left}
                  ...
                  {hoveredPoint && (
                    ...
                    >
                      ...
                      <strong
                        style={{ fontSize: "1.15rem", display: "block", color: "var(--ink)" }}
                      >
                        {formatCurrency(hoveredPoint.price, hoveredPoint.currency, localeTag)}
                      </strong>
                      <span
                        style={{ fontSize: "0.82rem", color: "var(--ink-muted)", display: "block" }}
                      >
                        {formatDateTime(hoveredPoint.date, localeTag)}
                      </span>
                      <span
                        className="history-tooltip-tag"
                        style={{ color: hoveredPoint.color, marginTop: "0.3rem", display: "block" }}
                      >
                        {hoveredPoint.label}
                      </span>
                    </div>
                  ...
                  >
                    <div className="compare-head">
                      ...
                      <div className="compare-badges">
                        {compareBadgesFromResponse.bestPriceId === card.watch_id ? (
                          <span className="compare-badge">
                            {t("watchlist.compare.bestPriceBadge")}
                          </span>
                        ) : null}
                        {compareBadgesFromResponse.stableId === card.watch_id ? (
                          <span className="compare-badge">
                            {t("watchlist.compare.mostStableBadge")}
                          </span>
                        ) : null}
                        {compareBadgesFromResponse.freshestId === card.watch_id ? (
                          <span className="compare-badge">
                            {t("watchlist.compare.freshestBadge")}
                          </span>
                        ) : null}
                      </div>
                    </div>
                    ...
                    <div className="compare-body">
                      <div>
                        ...
                        <strong>
                          {card.latest_price == null
                            ...
                            : formatCurrency(card.latest_price, card.currency, localeTag)}
                        </strong>
                      </div>
                      <div>
                      ...
                      <div>
                        ...
                        <strong>
                          {card.avg_price == null
                            ? t("watchlist.compare.noData")
                            : formatCurrency(card.avg_price, card.currency, localeTag)}
                        </strong>
                      </div>
                    </div>
                    <div className="compare-meta">
                      ...
                      <span>
                        {t("watchlist.compare.volatility", {
                          value: volatilityLabel(card.volatility_hint, t),
                        })}
                      </span>
                    </div>
                  </article>
