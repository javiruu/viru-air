...
export { DEFAULT_LOCALE, getLanguage, LANGUAGES, type Locale, type LocaleTag } from "./config";

type DictValue = string | { one: string; other: string };
type TranslationDictionary = { readonly [key: string]: unknown };
...
function getNestedValue(dict: TranslationDictionary, key: string): DictValue | null {
...
export function createI18n(dictionaries: Dictionaries) {
  function t(locale: Locale, key: string, params?: Record<string, string | number>): string {
    const dict = dictionaries[locale] || dictionaries[DEFAULT_LOCALE];
    const entry = getNestedValue(dict, key) ?? getNestedValue(dictionaries[DEFAULT_LOCALE], key);
    if (!entry) return key;
  ...
  function useI18n(rawLocale?: string | null) {
    ...
    return { locale, localeTag: localeTag(locale), t: translator };
  }

  return { t, translate, useI18n };
}
