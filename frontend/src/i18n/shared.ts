export const sharedEs = {
  actions: {
    back: "Volver",
    cancel: "Cancelar",
    close: "Cerrar",
    confirm: "Confirmar",
    continue: "Continuar",
    create: "Crear",
    delete: "Eliminar",
    edit: "Editar",
    save: "Guardar",
    submit: "Enviar",
    update: "Actualizar",
    retry: "Reintentar",
    open: "Abrir",
    enter: "Entrar",
    logout: "Cerrar sesión",
  },
  states: {
    loading: "Cargando…",
    saving: "Guardando…",
    updating: "Actualizando…",
    empty: "Sin datos todavía.",
    notAvailable: "No disponible",
  },
  notices: {
    success: "Hecho",
    error: "Algo falló",
  },
  time: {
    minutes: { one: "{count} min", other: "{count} min" },
    hours: { one: "{count} h", other: "{count} h" },
    days: { one: "{count} día", other: "{count} días" },
  },
  labels: {
    language: "Idioma",
    region: "Región",
    currency: "Moneda",
    timezone: "Zona horaria",
  },
  errors: {
    sessionExpired: "Sesión caducada. Inicia sesión de nuevo.",
    sessionRequired: "Necesitas iniciar sesión para continuar.",
    generic: "Se ha producido un error. Inténtalo de nuevo.",
  },
  a11y: {
    openAccountMenu: "Abrir menú de cuenta",
    accountMenu: "Menú de cuenta",
    changeTheme: "Cambiar tema",
    changeLanguage: "Cambiar idioma",
    mainNavigation: "Navegación principal",
    notificationsUnread: "{count} señales sin leer",
  },
  loader: {
    flightLabel: "cargando vuelo",
    aria: "Cargando",
  },
  theme: {
    dark: "Modo oscuro",
    light: "Modo claro",
  },
  notifications: {
    loginSuccessBody: "Tu panel privado ya está disponible.",
    registerSuccessBody: "Tu cuenta está activa y lista para usar.",
    sessionRequiredTitle: "Acceso requerido",
    sessionExpiredTitle: "Sesión caducada",
    dashboardAutoLoginTitle: "Cabina demo abierta",
    dashboardAutoLoginBody: "Viru entro con user@viru.local para este dashboard.",
    dashboardAutoLoginFailedTitle: "Demo no disponible",
    themeChangedTitle: "Tema actualizado",
    themeDarkBody: "Modo oscuro activado.",
    themeLightBody: "Modo claro activado.",
  },
  footer: {
    kicker: "Terminal Viru",
    publicBody:
      "Cabina pública de señales para seguir precios Ryanair con contexto, alertas y criterio de compra.",
    privateBody:
      "Tu espacio de trabajo en Viru para seguir rutas, alertas y decisiones con claridad.",
    statusLabel: "Contexto activo",
    statusPublic: "Ruta abierta",
    statusPrivate: "Espacio privado",
    copyLink: "Copiar enlace base",
    navLabel: "Navegación de pie de página",
    scrollTop: "Volver arriba ↑",
    note: "Rutas, señales y soporte oficial de Viru Tracker.",
    copyright: "© {year} Viru Tracker. Todos los derechos reservados.",
    groups: {
      explore: "Producto",
      access: "Acceso",
      contact: "Contacto",
      workspace: "Espacio de trabajo",
      account: "Cuenta",
      support: "Soporte",
      admin: "Administración",
    },
    links: {
      home: "Inicio",
      help: "Ayuda",
      policies: "Políticas",
      register: "Crear cuenta",
      supportEmail: "soporte@viru.app",
      privacyEmail: "privacidad@viru.app",
      pressEmail: "press@viru.app",
      dashboard: "Panel",
      watchlist: "Watchlist",
      doorToDoor: "Puerta a puerta",
      quickSearch: "Búsqueda rápida",
      hotelsRadar: "Radar hotelero",
      alerts: "Alertas",
      notifications: "Señales",
      opportunities: "Oportunidades",
      productFeedback: "Enviar opinion",
      profile: "Perfil",
      security: "Seguridad",
      preferences: "Preferencias",
      appearance: "Apariencia",
      searchPrefs: "Búsqueda",
      region: "Idioma y región",
      supportHelp: "Centro de ayuda",
      contact: "Contacto",
      feedback: "Reportar problema",
      aboutUs: "Sobre Viru",
      admin: "Panel de administración",
      productHealth: "Salud del producto",
    },
  },
  statusCatalog: {
    watch: {
      active: "Activo",
      paused: "Pausado",
      deleted: "Eliminado",
    },
    delivery: {
      queued: "En espera",
      sent: "Enviado",
      delivered: "Enviado",
      failed: "Error",
      error: "Error",
    },
    system: {
      ok: "Operativo",
      degraded: "Degradado",
      critical: "Incidencia",
    },
  },
};

export const sharedEn = {
  actions: {
    back: "Back",
    cancel: "Cancel",
    close: "Close",
    confirm: "Confirm",
    continue: "Continue",
    create: "Create",
    delete: "Delete",
    edit: "Edit",
    save: "Save",
    submit: "Submit",
    update: "Update",
    retry: "Retry",
    open: "Open",
    enter: "Sign in",
    logout: "Sign out",
  },
  states: {
    loading: "Loading…",
    saving: "Saving…",
    updating: "Updating…",
    empty: "No data yet.",
    notAvailable: "Not available",
  },
  notices: {
    success: "Done",
    error: "Something went wrong",
  },
  time: {
    minutes: { one: "{count} min", other: "{count} min" },
    hours: { one: "{count} hr", other: "{count} hrs" },
    days: { one: "{count} day", other: "{count} days" },
  },
  labels: {
    language: "Language",
    region: "Region",
    currency: "Currency",
    timezone: "Time zone",
  },
  errors: {
    sessionExpired: "Session expired. Please sign in again.",
    sessionRequired: "You need to sign in to continue.",
    generic: "Something went wrong. Please try again.",
  },
  a11y: {
    openAccountMenu: "Open account menu",
    accountMenu: "Account menu",
    changeTheme: "Change theme",
    changeLanguage: "Change language",
    mainNavigation: "Main navigation",
    notificationsUnread: "{count} unread signals",
  },
  loader: {
    flightLabel: "loading flight",
    aria: "Loading",
  },
  theme: {
    dark: "Dark mode",
    light: "Light mode",
  },
  notifications: {
    loginSuccessBody: "Your private panel is now available.",
    registerSuccessBody: "Your account is active and ready.",
    sessionRequiredTitle: "Sign-in required",
    sessionExpiredTitle: "Session expired",
    dashboardAutoLoginTitle: "Demo cockpit open",
    dashboardAutoLoginBody: "Viru signed in with user@viru.local for this dashboard.",
    dashboardAutoLoginFailedTitle: "Demo unavailable",
    themeChangedTitle: "Theme updated",
    themeDarkBody: "Dark mode enabled.",
    themeLightBody: "Light mode enabled.",
  },
  footer: {
    kicker: "Viru Terminal",
    navLabel: "Footer navigation",
    publicBody:
      "Public signal cockpit to track Ryanair fares with context, alerts, and better booking timing.",
    privateBody:
      "Your Viru workspace for tracking routes, alerts, and decisions with clarity.",
    statusLabel: "Active context",
    statusPublic: "Open route",
    statusPrivate: "Private space",
    copyLink: "Copy base link",
    scrollTop: "Back to top ↑",
    note: "Official Viru Tracker routes, signals, and support.",
    copyright: "© {year} Viru Tracker. All rights reserved.",
    groups: {
      explore: "Product",
      access: "Access",
      contact: "Contact",
      workspace: "Workspace",
      account: "Account",
      support: "Support",
      admin: "Admin",
    },
    links: {
      home: "Home",
      help: "Help",
      policies: "Policies",
      register: "Create account",
      supportEmail: "soporte@viru.app",
      privacyEmail: "privacidad@viru.app",
      pressEmail: "press@viru.app",
      dashboard: "Dashboard",
      watchlist: "Watchlist",
      doorToDoor: "Door to door",
      quickSearch: "Quick search",
      hotelsRadar: "Hotel radar",
      alerts: "Alerts",
      notifications: "Signals",
      opportunities: "Opportunities",
      productFeedback: "Product feedback",
      profile: "Profile",
      security: "Security",
      preferences: "Preferences",
      appearance: "Appearance",
      searchPrefs: "Search",
      region: "Language and region",
      supportHelp: "Help center",
      contact: "Contact",
      feedback: "Report issue",
      aboutUs: "About us",
      admin: "Admin panel",
      productHealth: "Product health",
    },
  },
  statusCatalog: {
    watch: {
      active: "Active",
      paused: "Paused",
      deleted: "Deleted",
    },
    delivery: {
      queued: "Queued",
      sent: "Sent",
      delivered: "Delivered",
      failed: "Error",
      error: "Error",
    },
    system: {
      ok: "Operational",
      degraded: "Degraded",
      critical: "Incident",
    },
  },
};
