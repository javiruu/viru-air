...
const outputDir = path.resolve(
  process.env.ADMIN_HOTELS_OUTPUT_DIR || "../docs/qa/evidence/h41-admin-hotels-observability",
);
const scenarios = [
...
const metrics = [
  ...
  },
  {
    ...
    outcome: "retried",
    count: 2,
    updated_at: "2026-08-09T12:11:00",
  },
  {
    metric_date: "2026-08-08",
    metric_name: "alert_event",
    provider: "mock",
    outcome: "created",
    count: 7,
    updated_at: "2026-08-08T18:20:00",
  },
  {
    metric_date: "2026-08-08",
    metric_name: "hotel_delivery",
    ...
    outcome: "failed",
    count: 1,
    updated_at: "2026-08-08T18:21:00",
  },
];
...
async function runScenario(scenario) {
  ...
  await page.route("**/api/v1/notifications/summary", (route) =>
    route.fulfill(responseJson({ unread: 0 })),
  );
  await page.route("**/api/v1/admin/hotels/runs**", (route) =>
    route.fulfill(
      responseJson({
        limit: 8,
        runs: [
          {
            ...
            items_processed: 18,
            has_error: false,
            outcomes: { snapshots_created: 18 },
          },
          {
            provider: "mock",
            status: "partial",
            started_at: "2026-08-09T11:10:00",
            finished_at: "2026-08-09T11:10:03",
            duration_seconds: 3,
            items_processed: 4,
            has_error: true,
            outcomes: { provider_fetch_failed: 1 },
          },
        ],
      }),
    ),
  );
  await page.route("**/api/v1/admin/hotels/provider-outcomes**", (route) =>
    route.fulfill(
      responseJson({
        limit: 20,
        ...
        providers: [
          {
            provider: "local",
            runs: 2,
            statuses: { running: 0, completed: 1, partial: 1, failed: 0, skipped: 0, unknown: 0 },
            outcomes: { offers_scanned: 5, snapshots_created: 3, provider_fetch_failed: 1 },
          },
        ],
  ...
  await page.route("**/api/v1/admin/hotels/sweep-leases**", (route) =>
    route.fulfill(
      responseJson({
        limit: 20,
        generated_at: "2026-08-09T12:12:00",
        ...
        leases: [
          {
            state: "expired",
            attempt_count: 2,
            lease_expires_at: "2026-08-09T12:00:00",
            finished_at: null,
          ...
          },
          {
            ...
            finished_at: "2026-08-09T11:00:00",
            last_error_code: null,
            has_provider_run: false,
            attention: false,
  ...
  await page.route("**/api/v1/admin/hotels/provider-controls**", (route) =>
    route.fulfill(
      responseJson({
        limit: 50,
        budgets: [
          {
            ...
            window_key: "2026-08-09",
            hard_limit: 10,
            units_reserved: 2,
            units_used: 3,
            units_released: 1,
            units_remaining: 5,
            window_expires_at: "2026-08-10T00:00:00",
            source: "local_config",
          },
        ],
        circuits: [
          {
            ...
            failure_threshold: 3,
            opened_at: "2026-08-09T12:00:00",
            next_probe_at: "2026-08-09T12:05:00",
  ...
  await page.route("**/api/v1/admin/hotels/health**", (route) =>
    route.fulfill(
      responseJson({
        status: "degraded",
        generated_at: "2026-08-09T12:12:00",
        ...
        providers: [
          {
            ...
            skipped: 0,
            deliveries_failed: 0,
            last_run_at: "2026-08-09T12:10:00",
            last_run_status: "completed",
            last_finished_at: "2026-08-09T12:10:01",
            age_seconds: 121,
          },
          {
            provider: "makcorps",
  ...
  await page.route("**/api/v1/admin/hotels/observability**", async (route) => {
    ...
    await route.fulfill(
      responseJson({ days: Number(url.searchParams.get("days") || 7), metrics: filtered }),
    );
  });

  await page.goto(`${baseUrl}/admin/hotels-observability`, {
    waitUntil: "networkidle",
    timeout: 60_000,
  });
  await page
    .getByRole("heading", { name: /Observabilidad hotelera|Hotel observability/i })
    .waitFor({ state: "visible", timeout: 30_000 });
  await page
    .getByRole("heading", { name: /Ledger operativo|Operational ledger/i })
    .waitFor({ state: "visible", timeout: 30_000 });
  await page
    .getByRole("heading", { name: /Runs recientes|Recent runs/i })
    .waitFor({ state: "visible", timeout: 30_000 });
  await page
    .getByRole("heading", { name: /Leases del sweep|Sweep leases/i })
    .waitFor({ state: "visible", timeout: 30_000 });
  await page
    .getByRole("heading", { name: /Outcomes por provider|Provider outcomes/i })
    .waitFor({ state: "visible", timeout: 30_000 });
  ...
  const titleVisible = await page
    .getByRole("heading", { name: /Observabilidad hotelera|Hotel observability/i })
    .isVisible();
  const healthRequestCount = requests.filter((request) =>
    request.url.startsWith("/api/v1/admin/hotels/health"),
  ).length;
  const runsRequestCount = requests.filter((request) =>
    request.url.startsWith("/api/v1/admin/hotels/runs"),
  ).length;
  const controlsRequestCount = requests.filter((request) =>
    request.url.startsWith("/api/v1/admin/hotels/provider-controls"),
  ).length;
  const controlsVisible = await page.locator(".hotel-observability-controls").isVisible();
  const leasesRequestCount = requests.filter((request) =>
    request.url.startsWith("/api/v1/admin/hotels/sweep-leases"),
  ).length;
  const leasesVisible = await page.locator(".hotel-observability-leases").isVisible();
  const outcomesRequestCount = requests.filter((request) =>
    request.url.startsWith("/api/v1/admin/hotels/provider-outcomes"),
  ).length;
  ...
  const focusVisible = await providerSelect.evaluate(
    (element) => document.activeElement === element,
  );
  await providerSelect.selectOption("local");
  const filteredResponse = page.waitForResponse(
    (response) =>
      response.url().includes("/admin/hotels/observability") &&
  ...
  const filterWorked =
    filteredRows > 0 && providerCells.every((provider) => provider.trim() === "local");
...
report.failedAssertions = report.scenarios.reduce(
  (sum, scenario) => sum + scenario.failedAssertionCount,
  0,
);
await fs.writeFile(
