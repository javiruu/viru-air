...
const outputDir = path.resolve(
  process.env.GATE_R_OUTPUT_DIR || "../docs/qa/evidence/hotels-h36-gate-r",
);
...
page.on("request", (request) => {
  ...
  if (request.method() !== "GET") {
    readOnlyViolations.push({
      method: request.method(),
      path: redactHotelPath(url.pathname),
...
try {
  ...
  const _submit = page.locator('[data-testid="hotel-search-submit"]');
  ...
  await page
    .locator(".hotel-area-suggestion-item")
    .first()
    .waitFor({ state: "visible", timeout: 30_000 });
  ...
  addScenario("area-autocomplete-search", {
    status: areaStatus,
    suggestionPresent,
    resolved,
    areaCount,
  ...
  await page.route("**/api/v1/hotels/search**", (route) =>
    route.fulfill({
      status: 503,
  ...
  await page.route("**/api/v1/hotels/*/parity", (route) =>
    route.fulfill({
      status: 503,
      contentType: "application/json",
  ...
  await page
    .locator(".hotel-parity-signal .status-pill.error")
    .waitFor({ state: "visible", timeout: 30_000 });
  ...
  await page.route("**/api/v1/hotels/search**", async (route) => {
    ...
    try {
      ...
      /* navigation cancelled the request */
    }
  });
  ...
  addScenario("navigation-cancel", {
    ...
    alertPresentAfterNavigation:
      (await page.locator('#main-content > .notice[role="alert"]').count()) > 0,
  });
} finally {
  if (traceEnabled) {
  ...
  const gateAssertions = [
    ...
    scenario("controlled-error")?.alertPresent === true &&
      scenario("controlled-error")?.queryRetained === true,
    scenario("controlled-empty")?.emptyPresent === true &&
      scenario("controlled-empty")?.alertCount === 0,
  ...
  await fs.writeFile(
    path.join(outputDir, "gate-r.json"),
    `${JSON.stringify(
      {
  ...
  await browser.close();
  if (unexpectedConsoleErrors.length > 0 || gateAssertions.some((passed) => !passed))
    process.exitCode = 1;
}
