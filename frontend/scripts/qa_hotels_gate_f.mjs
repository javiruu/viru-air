...
const outputDir = path.resolve(
  process.env.GATE_F_OUTPUT_DIR || "../docs/qa/evidence/hotels-h36-gate-f",
);
const _consentKey = "viru_hotels_rum_consent";
...
function validatePayload(payload) {
  ...
  if (
    ...
    Object.keys(metadata).length !== allowedKeys.size
  )
    failures.push("metadata-keys-not-allowlisted");
...
async function runScenario(name, consent, storage) {
  ...
  try {
    ...
    const response = await page.goto(`${baseUrl}/hoteles`, {
    ...
    await page
      .goto(`${baseUrl}/login`, { waitUntil: "domcontentloaded", timeout: 60_000 })
      .catch(() => undefined);
    await page.waitForTimeout(800);
...
try {
  ...
  if (/[?&](?:token|password|secret|access_token)=/i.test(serialized))
    violations.push("query-secret-marker");
  report.privacy.violations = violations;
  if (violations.length > 0)
    throw new Error(`gate_f_privacy_validation_failed:${violations.join(",")}`);
  await fs.writeFile(
    path.join(outputDir, "gate-f.json"),
