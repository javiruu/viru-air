...
async function mockApi(page) {
  await page.route("**/api/v1/**", async (route) => {
    ...
    if (apiPath === "/auth/me")
      return fulfill({ id: "qa-user", email: "qa@viru.local", locale: "es", is_admin: false });
    ...
    if (/^\/watchlist\/[^/]+$/.test(apiPath)) {
      ...
      return fulfill({
        ...watch,
        latest_snapshot: snapshots.find((snapshot) => snapshot.watch_id === watch?.id) ?? null,
        price_history: [],
      });
    }
    if (apiPath.startsWith("/prices/compare"))
      return fulfill({ currency_mode: "single", watches: [], points: [] });
    return fulfill({});
  });
}
...
try {
  ...
  const defaultBorder = await originField.evaluate(
    (element) => getComputedStyle(element).borderColor,
  );
  await originField.hover();
  assert.notEqual(
    await originField.evaluate((element) => getComputedStyle(element).borderColor),
    defaultBorder,
  );
  await origin.focus();
  assert.notEqual(
    await originField.evaluate((element) => getComputedStyle(element).boxShadow),
    "none",
  );
  await page.screenshot({
    path: path.join(screenshotsDir, "desktop-dark-full.png"),
    fullPage: true,
  });
  await page
    .locator(".watch-smart-tool-group--route-tools")
    .screenshot({ path: path.join(screenshotsDir, "desktop-dark-toolbar.png") });
  ...
  for (const scenario of [
    ...
    await run.page.screenshot({
      path: path.join(screenshotsDir, `${scenario.name}.png`),
      fullPage: true,
    });
    report.scenarios.push({
      name: scenario.name,
      overflow: await run.page.evaluate(
...
assert.ok(
  report.scenarios.every((scenario) => !scenario.overflow),
